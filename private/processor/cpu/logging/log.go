...
func (ls LogState) String() string {
	return fmt.Sprintf("PC: 0x%04X, SP: 0x%04X, OP: %s", ls.reg.PC, ls.reg.SP, ls.mnemonic)
}
...
func (l *Logger) Log(reg registers.Registers, mnemonic string) {
	l.logs = append(l.logs, LogState{reg, mnemonic})
}
...
func (l *Logger) Flush() {
	...
	currentSP := l.logs[0].reg.SP
	var group []LogState
	for _, entry := range l.logs {
		if entry.reg.SP == currentSP {
