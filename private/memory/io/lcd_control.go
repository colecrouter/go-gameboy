...
type LCDControl struct {
	...
	BackgroundUseSecondTileMap bool // Bit 3 - BG Tile Map Display Select     (0=9800-9BFF, 1=9C00-9FFF)
...
func (l *LCDControl) Read(addr uint16) uint8 {
	...
	if l.BackgroundUseSecondTileMap {
		val |= 1 << 3
	}
	if l.Sprites8x16 {
		val |= 1 << 2
...
func (l *LCDControl) Write(addr uint16, value uint8) {
	...
	l.BackgroundUseSecondTileMap = value&(1<<3) != 0
